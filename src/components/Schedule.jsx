...
function ShiftBlock({ shift, offset, last }) {
  ...
  return (
    <div
      ref={revealRef}
      className={`reveal rounded-2xl p-5 md:p-6 mb-6 md:mb-0 ${offset} ${
        last ? 'bg-primary shadow-cardHover' : 'bg-white border border-line'
      }`}
    >
      <p className={`text-xs font-bold tracking-widest ${last ? 'text-accent-soft' : 'text-primary'}`}>
        {shift.label.toUpperCase()}
      </p>
      <p className={`font-display font-extrabold text-xl md:text-2xl mt-2 ${last ? 'text-white' : 'text-ink'}`}>
        {shift.time}
      </p>
    </div>
  )
...
export default function Schedule() {
  ...
  return (
    <section id="jadval" className="py-20 md:py-28 bg-fog">
      <div className="max-w-7xl mx-auto px-5 sm:px-8">
        <div ref={headingRef} className="reveal max-w-xl">
          <span className="text-primary font-semibold text-sm tracking-wide">Jadval</span>
